---
name: interview-prep
description: Use when an interview is booked or invited and Kenny wants a prep doc, cheat sheet, or research on the company and the person interviewing him — "I have an interview with X", "prep me for this call", "who is this interviewer", "build the cheat sheet", "recruiter screen tomorrow". Also covers the post-interview debrief. Not for take-homes (take-home-strategist) or algorithm drilling (leetcode-coach).
---
...
Produce **one phone-open document** Kenny can read live during the call, plus a debrief after it.

The document is not a research dump. It is an operating manual for thirty to sixty minutes under pressure. Every section earns its place by being something he would otherwise have to recall while talking.

**Core principle: match the doc to what the round can actually decide.** A recruiter screen cannot be won — it can only be lost, and it is the cheapest place to extract level, band, and format. A hiring-manager or founder round is most of the decision. Prep for the round in front of him, not for "the interview."
...
4. **Map him onto the req.** Rank the strengths that actually win this job. Name the gaps and write the honest one-liner for each. Never invent experience; never bluff a stack — bluffs compound in later rounds.
5. **Write the doc.** See `templates.md`. Use the section set that matches the round; drop sections that do not apply rather than padding them.
6. **Debrief after.** See `templates.md`. Facts learned, what to fix, action items. This is what makes the *next* round's doc good.
...
- **Confirm the format before building a deep-technical section.** Public interview write-ups often describe a *different team* at the same company. Prepping database internals for a product-UI role burns days.
...
- **Private reads live in their own section, clearly labeled not-for-the-call.** Skepticism about the company belongs in the doc; it does not belong in his mouth.
...
- **Never badmouth a past employer.** Some formats (Topgrading) exist specifically to catch it. A layoff line is one breath, factual, warm, move on.
...
- Untagged claims. If it is not tagged, he cannot tell what is safe to say out loud.
- You built a technical deep-dive before confirming the round is technical.
- The doc is long but unnavigable mid-call — no jump nav, no ordering, no bolding of the line he actually has to say.
