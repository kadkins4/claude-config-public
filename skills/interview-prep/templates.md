...
The cheat sheet is read live, mid-call, under pressure. Optimize for retrieval, not completeness. Bold the sentence he actually has to say. Drop any section that does not apply — an empty section is worse than a missing one.
...
```markdown
---
tags: [career, job-search-2026, <company-slug>, <round-type>, cheat-sheet]
company: <Company>
...
Plain-English product description first — the sentence he could say to a friend. Then: funding, scale, ARR/headcount, trajectory, notable customers, competitors. Then **the specific team or product surface** this req sits on, and what the job therefore actually is day to day. Close with one line: why this fits, in one breath.
...
1. **Now:** current status in one sentence — including the layoff line if relevant. Zero drama, move on.
...
4. **The differentiator:** ownership, with the one real metric.
...
# 🔨 HAMMER THESE HOME
*If the call ends and none of these landed, it went badly.* Five or six, none long. Include the one hard metric and spend it deliberately — do not scatter it.
...
Where he will physically be, connection, backup, what time to be ready, who to email if running late, and any conflicts on the same day. Availability already sent — read from here, do not re-derive on the phone.
...
```markdown
---
tags: [career, job-search-2026, <company-slug>, interview-debrief]
company: <Company>
...
## What to fix
Each item: what happened, then the concrete fix in his own words for next time. Not self-flagellation — a scripted replacement line.
...
- **Confidence tags everywhere.** `[CERTAIN]` from a primary source, `[LIKELY]` inferred, `[GUESSING]` filling a gap. He needs to know what is safe to say out loud.
- **Blockquote anything he says verbatim.** Scripted lines get quote formatting so they are findable at a glance.
- **✍️ marks answers only he can supply.** Never improvise his personal history — flag it and let him fill it in before the call.
